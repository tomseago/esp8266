...
void WebSocketsClient::asyncConnect() {
    ...
    tcpclient->onDisconnect([](void *obj, AsyncClient* c) {
        DEBUG_WEBSOCKETS("[WS-Client] onDisconnect c=%p\n", c);
        c->free();
        delete c;
    });
    ...
    DEBUG_WEBSOCKETS("[WS-Client:TS] Calling tcpClient->connect\n");
    if(!tcpclient->connect(_host.c_str(), _port)) {
        DEBUG_WEBSOCKETS("[WS-Client:TS] tcpclient->connect failed, will kill tcpclient\n");
        connectFailedCb();
        delete tcpclient;
    }
